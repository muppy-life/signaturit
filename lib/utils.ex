defmodule Signaturit.Utils do
  ...
  def reduce_keys_to_atoms({key, val}) when is_map(val),
    do: {handle_to_existing_atom(key), keys_to_atoms(val)}

  def reduce_keys_to_atoms({key, val}) when is_list(val),
    do: {handle_to_existing_atom(key), Enum.map(val, &keys_to_atoms(&1))}
  ...
  def fill_array(array, params, parent) when is_map(params) or is_list(params) do
    iterable =
      if is_map(params),
        do: Map.to_list(params),
        else: Enum.with_index(params, fn element, index -> {index, element} end)
